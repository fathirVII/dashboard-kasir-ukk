<x-app>
    ...
    <div class=" bg-[#262537] px-6 py-4 mt-4 rounded-2xl shadow-md">
        <div class="">
            <h1 class="text-lg font-bold uppercase mb-4">Detail Penjualan</h1>

            <div class="text-center mb-4">
                <h1 class="text-xl font-bold">Struk Penjualan</h1>
            ...
            <div class="border-t border-b py-2 my-2">
                ...
                @foreach ($penjualan->detail as $detail)
                    <div class="flex justify-between text-sm text-gray-200 mt-1">
                        <span class="w-1/3 truncate">{{ $detail->produk->nama }}</span>
                        <span class="w-1/3 text-center">{{ $detail->jumlah_barang }} x
                            {{ number_format($detail->produk->harga) }}</span>
                        <span class="w-1/3 text-right">{{ number_format($detail->sub_total) }}</span>
                    </div>
                    @php
